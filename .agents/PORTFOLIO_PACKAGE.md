...
> **Readiness hold — 2026-07-22:** 이 문서는 포트폴리오 아이디어와 미래 절차를
> 보존한 계획 자료다. B2는 개발 기준선일 뿐 release 또는 publication 권한이 아니다.
> 과거 버전의 `즉시`, `3분`, `투고만` 같은 표현은 실행 승인이 아니며, 현재는
> GitHub tag/release, Zenodo DOI, preprint/학회 제출, 외부 연락을 수행하지 않는다.
> REL-01의 immutable release와 PUB-01의 metadata/venue 검증을 통과하고 owner가
> 별도로 승인한 뒤에만 관련 절차를 실행한다. 현재 권한은
> [`USER_ACTIONS.md`](USER_ACTIONS.md)가 우선한다.
...
| Zenodo DOI | ⛔ PUB-01까지 차단 | 레포를 인용 가능한 영구 학술 산출물로 격상 (CERN 운영, 무료) | immutable REL-01 + owner 승인 |
| 블로그 글 (실패 갤러리) | ✅ 내부 초안 `outreach/blog_failure_gallery_ko.md` | "설명하는 능력" 증명 + 초보자 도달 후보 | owner 게시 승인 |
| engrXiv/EdArXiv 프리프린트 | ⛔ 현재 제출 금지 | 무료 DOI + Google Scholar 색인 후보 | PUB-01 재검증 + owner 승인 |
| arXiv 프리프린트 | ⛔ 현재 제출 금지 | 공개 학술 문서 후보 | PUB-01 재검증 + owner 승인 |
| JOSE 논문 | 🟨 후보 자료 보존 | "단독 논문 게재" 경험 후보 | 최신 eligibility·원고 수치·release identity 재검증 |
| YouTube 실물 데모 | ⬜ 2027 로드맵 | 영상 포트폴리오 (실물 RB-Y1 — 익명 유지) | 실물 데이터 획득 후 |
| KRoC 발표 | ⬜ 2027.2 | "학회 발표" 경험 + 네트워킹 | 마감 공지(감시 봇) |

## 미래 Zenodo 연결 절차 — 현재 실행 금지

Zenodo 연동은 REL-01이 immutable release와 checksums/SBOM/provenance를 만든 뒤,
PUB-01에서 metadata 일치를 확인하고 owner가 별도 승인할 때만 사용한다. 아래는 그
시점에 공식 절차를 다시 검증하기 위한 historical checklist다.

1. https://zenodo.org 에 GitHub 계정으로 로그인
2. 상단 계정 → **GitHub** 설정 → 저장소 목록에서
   `ycpiglet/manipulator-control-tutorial` 토글을 **ON**
3. 승인된 REL-01 exact tag에서만 GitHub prerelease/release를 생성하고, Zenodo가
   정확히 같은 commit/version을 아카이빙했는지 검증
4. 발급된 DOI와 archived SHA가 일치할 때만 별도 reviewed PR로 README와
   `CITATION.cff` metadata를 갱신

`CITATION.cff`가 있어도 내용이 자동으로 정확하다고 가정하지 않는다. PUB-01에서
저자·ORCID·제목·버전·license·repository·release SHA를 대조한다.
...
블로그 공개가 향후 venue 정책에 영향이 없다고 가정하지 않는다. 게시 전 대상 venue의
최신 prior-publication 정책을 재확인하고, 필요한 경우 cover letter에 선공개 사실을
고지한다.
...
### Track C 미래 후보 — 실행은 PUB-01 이후

1. **Zenodo** — immutable release와 review PDF의 영구 인용 후보.
2. **engrXiv 또는 EdArXiv** — 무료 DOI 프리프린트로 "첫 단독 논문" 경험을
   무료 후보. arXiv 대안이자 교육 프레이밍에 더 맞을 수 있으나, U3/U5 적합성과
   현재 정책은 제출 시점에 다시 검증한다.
...
- 2026-07-07 (야간): 블로그 영문판 완성(dev.to/Medium용), U5-EN 11쪽 완성으로
  P3(engrXiv 프리프린트)용 후보 원고를 준비함. 이 기록은 제출 준비도나 현재
  venue 적합성 판정이 아니다.
- 2026-07-22: STATE-01 readiness hold 추가. 모든 외부 게시·release·DOI·submission은
  해당 gate와 owner 승인까지 보류.

## 사용자 액션 — 현재 보류

- P1: REL-01과 PUB-01 뒤 owner가 승인하면 Zenodo 연동과 exact release archive를
  검증한다. 지금은 토글, tag, release, DOI 작업을 하지 않는다.
- P2: 블로그 게시도 외부 publication action이다. 플랫폼 선택과 게시 전 owner의
  명시적 지시 및 venue-policy 재검증을 기다린다.

...
- 상태: **Execution baseline v14 — STATE/IA/INT accepted; SUP-01A/B/C accepted; SUP-01D next**
- 작성일: 2026-07-23 KST
...
- B1 선언 시점에는 없던 branch ruleset, vulnerability/security settings, immutable
  Action pin은 GOV-01로 완료됐다. 이후 SUP-01A/B/C가 reviewed Node24 Action lock,
  universal hash-locked Python profiles, pinned Panda runtime closure를 추가했다.
  Vulnerability/license scans, Linux system-package evidence, SBOM inputs,
...
| 9 | `agent/dependency-reproducibility` / SUP-01 | A/B/C 완료; scan/SBOM/system package 다음 | lock, pinned asset/action, scans |
...
historical evidence로 보존됐다. 현재 7a/7b/8a/8b disposition은 끝났다. 남은 SUP-01과
PKG-01, OPS-01, EDU-01, MAINT-01은 파일 ownership과 registry prerequisite가 겹치지
않을 때 병렬로 진행할 수 있다. E2E-01과 LIC-01은 각각 표의 SUP/PKG 및 SUP 선행
조건을 유지한다. 표의 순서는 위험과 최종 merge 의존성을 뜻하며 모든 작업을
직렬화하라는 의미는 아니다.
...
7. B2 declaration의 `N/A — SUP-01 pending`은 record-creation 당시 사실로 보존한다.
   이후 lock과 Panda pin의 accepted 상태는 append-only finding event와 live state가
   기록하며 B2 record를 소급 수정하지 않는다.
...
- STATE-01, IA-00 FREEZE, INT-01 disposition과 launcher in-place hardening은
  accepted evidence를 갖췄다. SUP-01A/B/C도 병합됐으며, 현재 critical lane은
  scan/SBOM/Linux system-package evidence를 닫는 SUP-01 후속이다. PKG-01은
  IA/INT 선행 조건이 끝나 독립 clean branch에서 진행할 수 있지만 workflow와
  packaging 최종 변경은 SUP/PKG 간 순차 merge한다. E2E-01은 두 aggregate가
  accepted된 exact main에서만 시작한다. OPS/EDU/MAINT는 파일 ownership이
  겹치지 않는 repo-only 범위에서 병렬화할 수 있다. 완료된 GOV-01, SAFE-01,
  DOC-01, COMP-01/02, BASE-01은 설정·안전·계약 회귀 감시만 유지한다.
...
1. Accepted `main@a216ee8b326008ffeae3e139fad65716c19ed341`까지의 STATE-01,
   IA-00, INT-01, SUP-01A/B/C evidence를 live state와 append-only finding event로
   reconcile한다. B2 records, 과거 archive/event와 감사 원문은 수정하지 않는다.
2. 그 protected-main reconciliation 뒤 clean worktree에서 SUP-01의 남은
   vulnerability/license scan, Linux system-package inventory/reproducibility, SBOM
   input contract를 구현한다. Scanner outage, malformed input, expired waiver,
   inventory drift는 fail closed해야 한다.
3. Accepted SUP inventory 뒤 LIC-01을 수행한다. Component별 SPDX/copyright/text/source
   coverage와 Qt/PySide LGPL 결정을 기록하되 외부 license 검토자 연락은 owner 승인
   전까지 하지 않는다.
4. IA-00/INT-01 prerequisites가 끝났으므로 PKG-01은 별도 clean branch에서 archive
   <=300 MiB, one-folder <=400 MiB, packaged cold-start와 actual-QML gate를 닫는다.
   E2E-01은 SUP-01과 PKG-01이 병합된 exact main에서 수행한다.
...
7. B2나 내부 SUP/PKG/OPS/EDU 준비를 public-beta, signed distribution, release/DOI, 실제 cleanup,

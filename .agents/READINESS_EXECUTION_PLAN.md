...
2026-07-21 현재 Lane C의 GOV-01A/B/C와 Lane A의 SAFE-01은 완료됐다.
...
수행하지 않았다. 다음 merge lane은 DOC-01이며 COMP-01/02는 그 뒤를 따른다.
...
- 동시에 진행 가능한 것: DOC-01 준비와 signing/license/pilot의 외부 준비.
  DOC-01→COMP-01→COMP-02는 계약 순서대로 병합하며, 완료된 GOV-01과 SAFE-01은
  설정·안전 회귀 감시만 유지한다.
...
3. DOC-01 병합 뒤 COMP-01 contract matrix와 COMP-02 소비자 변경을 각각 별도 PR로
   진행한다.
4. GOV-01, SAFE-01, DOC-01, COMP-01/02가 모두 통과하기 전에는 B2를 선언하지 않는다.
5. 실제 outputs root dry-run은 DOC-01과 분리해 owner가 참여할 때만 기본 dry-run으로

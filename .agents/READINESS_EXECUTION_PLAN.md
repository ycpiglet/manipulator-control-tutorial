...
- 상태: **Planning baseline v1 — 구현 착수 전**
- 작성일: 2026-07-20 KST
- 기준 원격: `origin/main@44b1937f3d34dcb5672b5aedc1df13fa55dfeaad`
- 계획 보존 브랜치: `agent/readme-project-handoff`
- 관련 PR: [#35](https://github.com/ycpiglet/manipulator-control-tutorial/pull/35)
...
1. PR #35로 README, 문서 진입점, 감사 기록, 인수인계 기준선을 먼저 병합한다.
2. 데이터 손실 위험이 있는 `mclab clean`과 잘못된 학습 완료 판정을 별도의 P0 PR로
   수정한다.
3. 두 P0 수정과 저장소 거버넌스가 통과한 깨끗한 `main`을 안전 기준선으로 선언한다.
4. 현재 오래되고 더러운 로컬 작업 트리의 패키징·폰트 변경은 파일 묶음별로 다시
   검토하여 새 기준선에 재적용한다. 작업 트리 전체를 병합하지 않는다.
5. 패키지 E2E, 의존성 고정, 라이선스, SBOM, 지원·개인정보 안내를 통과시켜 제한된
   기술 프리뷰와 source-first public beta를 구분한다.
6. 실제 장비·보조기술·초보자 검증과 서명·공증·롤백을 통과한 뒤에만 일반 사용자를
   위한 정식 바이너리 배포로 승격한다.
7. 불변 릴리스를 만든 뒤 Zenodo DOI와 출판 메타데이터를 갱신한다. JOSE는 접수 재개를
...
- PR #35는 `origin/main@44b1937`에서 만든 깨끗한 브랜치이며 감사 기록과 KR/EN
  README 정리를 포함한다.
- 본 계획 추가 전 PR #35의 문서 head
  `5cfe3c1459b90f45b024d8e1bfd3a8e4a4e1277f`에서 simulator, paper 2개,
  unsigned desktop 3개 등 6개 check가 모두 통과했다. 계획 commit은 같은 check를
  다시 통과해야 한다.
- 최신 main의 기본 품질 근거는 437 passed, 2 skipped, 1,088 subtests, Ruff 0,
  statement coverage 81.02%다.
- `mclab clean` 대상 선택은 임의 output root와 모든 자식 디렉터리를 허용하며,
  현재 출력 폴더를 대상으로 한 비삭제 시뮬레이션에서 learner output 전체를 삭제
  후보로 선택했다.
...
- 현재 GitHub main에는 branch protection/ruleset이 없고, vulnerability alerts,
  dependency lock, immutable release, tag가 준비되지 않았다.
...
| 기준선 | 선언 시점 | 내용 | 승격 조건 |
|---|---|---|---|
| B0 `audit-docs` | 현재 | main `44b1937` + PR #35의 live head | G0 |
| B1 `newcomer-baseline` | PR #35 병합 후 | README/IA/audit/handoff가 포함된 exact main SHA | G0 evidence 보존 |
| B2 `safe-main` | P0와 거버넌스 후 | safe cleanup + shared completion + protected main | G1 |
...
1. 모든 구현 브랜치는 PR #35 병합 후 최신 `origin/main`에서 새 clean worktree로 만든다.
...
3. P0 PR은 fixture/temp directory만 사용한다. 실제 `outputs/`에 destructive test를
   실행하지 않는다.
4. `mclab clean`은 SAFE-01 병합 전까지 사용자 안내와 검증 명령에서 금지한다.
...
| GOV-01 | Repository admin + Security lead | DOC-00 | S/M | protected main, required checks/review, CODEOWNERS, SECURITY/SUPPORT, Dependabot, SHA-pinned Actions |
...
| COMP-01 | Safety/Pedagogy maintainer | DOC-00 | M | read-only canonical completion evaluator, versioned reason, legacy/golden contract fixtures |
...
| BASE-01 | Release owner | GOV-01, SAFE-01, COMP-02 | S | clean current-main SHA, zero open P0, evidence manifest |
...
| IA-01 | Developer experience owner | BASE-01 | M | launcher consolidation with one-release shims; no config/model/publication moves |
...
|---|---|---|---|
| 0 | 기존 `agent/readme-project-handoff` / DOC-00·본 계획 | 6 checks + human review | B1 선언 |
| 1 | `agent/repository-governance` / GOV-01 | owner의 setting 권한 | policy files + settings snapshot |
| 2 | `agent/safe-output-cleanup` / SAFE-01 | B1 | cleanup을 다시 허용하거나 command disabled 유지 |
| 3 | `agent/completion-contract` / COMP-01 | B1 | 아직 소비자 동작은 바꾸지 않는 canonical evaluator |
| 4 | `agent/completion-consumers` / COMP-02 | COMP-01 | 모든 surface parity |
| 5 | `agent/safe-main-baseline` / BASE-01 | GOV/SAFE/COMP | B2 선언서만 기록 |
| 6a | `agent/font-assets` / INT-01 font | B2, owner intent 확인 | 최소 hunk 채택 또는 superseded로 종료 |
| 6b | `agent/package-size` / INT-01 package | B2 | package exclusion과 측정치만 채택 |
| 7 | `agent/dependency-reproducibility` / SUP-01 | B2 | lock, pinned asset/action, scans |
| 8 | `agent/third-party-notices` / LIC-01 | SUP-01 inventory | notices와 compliance 승인 |
| 9 | `agent/package-release-metrics` / PKG-01 | B2/INT 결정 | size/startup contract |
| 10 | `agent/packaged-e2e` / E2E-01 | SUP/PKG | supported OS E2E evidence |
| 11 | `agent/support-privacy-retention` / OPS-01 | B2 | 운영·데이터 보호 문서/검증 |
| 12 | `agent/educator-pilot-kit` / EDU-01 | B2 | protocol/guide; pilot 자체는 외부 일정 |
| 13 | `agent/beta-release` / REL-01 | G3 | B4 prerelease 또는 NO-GO 기록 |
| 14 | `agent/platform-accessibility-validation` / HUM-01 | E2E/EDU | 실제 장비·pilot evidence |
| 15 | `agent/signed-release` / REL-02 | G3 + credentials | B5 또는 NO-GO 기록 |
| 16 | `agent/publication-release-sync` / PUB-01 | immutable release | B6/DOI; JOSE는 external-blocked 가능 |
| 17 | `agent/launcher-compatibility` / IA-01 | B2, preferably B4 | root shim을 둔 launcher 정리 |
| 18+ | 작은 `agent/maint-*` / MAINT-01 | B2 | type/module/config debt를 작은 revert 단위로 감소 |

6a와 6b, 7과 9, 11과 12는 파일 ownership이 겹치지 않으면 병렬로 진행할 수 있다.
표의 순서는 위험과 최종 merge 의존성을 뜻하며 모든 작업을 직렬화하라는 의미는 아니다.

## 7. 단계별 실행 순서

### Wave 0 — 문서·인수인계 기준선 고정
...
완료 조건: G0. 이 단계에서는 감사 P0 코드를 수정하지 않는다.
...
- Lane B1 `agent/completion-contract`: COMP-01
- Lane B2 `agent/completion-consumers`: COMP-02; COMP-01 병합 뒤 시작
- Lane C `agent/repository-governance`: GOV-01과 필요한 GitHub setting change record

SAFE-01, COMP-01, GOV-01은 코드상 독립 PR로 개발할 수 있다. 권장 merge 순서는
GOV-01의 보호 규칙 확인, SAFE-01, COMP-01, COMP-02다. 저장소 setting에 필요한 owner
승인이 늦어지더라도 SAFE-01 review는 계속하되 B2를 선언하지 않는다. 각 후속 PR은
최신 main으로 rebase한다. GOV-01의 required checks는 현재 6개 check name과 admin
recovery 절차를 확인한 뒤 켠다.
...
2. 감사 당시 dirty checkout의 변경을 다음처럼 분리하여 owner가 의도를 확인한다.
   - package-size 묶음: `packaging/mclab.spec`, `tests/test_launch_scripts.py`
   - font 묶음: `src/mclab/application/qt_app.py`, Noto README,
     font build script/module/test/static assets
3. 오래된 `qt_app.py` 전체를 복사하지 않는다. 현재 main diff에 필요한 hunk만 재적용한다.
4. `agent/font-assets`와 `agent/package-size` PR을 각각 검증한다.
5. exact SHA, dependency/model hashes와 기본 CI를 B2에 기록한다.

완료 조건: B2가 clean이고 open P0가 0개다. 더러운 checkout은 참고 자료일 뿐 merge
source가 아니다.
...
| KR/EN contract | semantic H2와 공개 명령 100% 대응 | 현재 9/9 수동 + 향후 `check_readme_contract.py` | 양쪽 동시 수정 |
| docs links | local links/anchors error 0 | 현재 changed-scope manual check 0 error + 향후 offline checker | broken link 수정 |
| quickstart truth | clean setup, doctor, Lab01 exit 0 and promised artifacts 100% | OS별 log | 지원 문구 수정 또는 flow 복구 |

README/CLI 변경 때 D1/D2를 PR-blocking 자동화하는 작업은 DOC-01로 GOV-01과 함께
추가한다. 자동화 전까지 PR #35의 수동 결과를 증거로 남긴다.
...
| cleanup intent | default dry-run; exact resolved plan; confirmation 전 deletion 0 | plan JSON and CLI tests | safe default 복구 |
| cleanup result | plan/execution set 동일; deletion failure silent 0; recovery path verified | temp trash/staging test | non-zero exit, recover/revert |
...
|---|---|---|---|
| learner data loss | cleanup이 invalid/internal/root를 선택 | SAFE-01 dry-run, root guard, mixed tests | command disable, staging 복구, PR revert |
...
- 동시에 진행 가능한 것: SAFE-01, COMP-01, GOV-01 초안, signing/license/pilot의 외부
  준비.
...
- `src/mclab/application/qt_app.py`, packaging spec, workflow는 한 시점에 한 PR owner만
  가진다.
- structure/launcher PR은 safety/package baseline 이후 진행하며 config/model/paper
  경로를 이동하지 않는다.
...
1. 이 계획을 PR #35에 추가하고 6개 check를 다시 통과시킨다.
2. owner가 PR #35의 newcomer flow와 본 계획을 review한다.
3. PR #35를 병합하고 B1 exact SHA를 기록한다.
4. 최신 main clean worktree에서 `agent/safe-output-cleanup`을 만든다.
5. SAFE-01 실패 fixture부터 작성하고 실제 `outputs/`에는 손대지 않는다.
6. 동시에 별도 owner가 COMP-01 contract matrix와 GOV-01 setting 변경안을 준비한다.

**PR #35 병합 전에는 감사 P0 구현을 시작하지 않는다. SAFE-01이 G1을 통과하기 전에는
`mclab clean`을 실행하지 않는다.**

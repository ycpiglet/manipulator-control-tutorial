...
- 상태: **Execution baseline v2 — GOV-01A/B 통과, GOV-01C 검증 중**
- 작성일: 2026-07-20 KST
- 현재 기준 원격: `origin/main@9f4169f60efb32d6b6d49b9f06d985d8de9c6f70`
...
1. GOV-01C에서 workflow의 third-party Actions를 reviewed full commit SHA로 고정하고
   replacement runs가 모두 green인지 확인한다.
2. GOV-01C 병합 뒤 `sha_pinning_required=true`를 적용하고 API와 main workflow를 다시
   검증한다.
3. 최신 main clean worktree에서 SAFE-01 실패 fixture부터 작성하고 실제 `outputs/`에는
   손대지 않는다.
4. 별도 clean worktree에서 COMP-01 contract matrix를 준비하되 COMP-02 소비자 변경과
   섞지 않는다.
5. GOV-01A/B/C, SAFE-01, COMP-01/02가 모두 통과하기 전에는 B2를 선언하지 않는다.

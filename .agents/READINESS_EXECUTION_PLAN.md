...
- 상태: **Execution baseline v4 — GOV-01 통과, SAFE-01 final review·exact-head gate 대기**
- 작성일: 2026-07-21 KST
- 현재 기준 원격: `origin/main@41be887f21bfb476507d94a089f98c0ef72453c8`
...
- 현재 `main`의 `mclab clean`은 아직 감사 당시의 임의 output root/모든 자식 선택
  동작을 포함한다. SAFE-01 로컬 후보는 이를 strict root/manifest selection, 기본
  dry-run, exact plan 적용, 복구 가능한 quarantine으로 교체했지만 아직 병합되지 않았다.
- SAFE-01 로컬 후보는 lexical root/ancestor pin, cross-process operation lock,
  ...
  없이 scoped GO를 냈다. frozen-tree Python 3.12는 528 passed, 6 skipped,
  1,109 subtests와 coverage 80.92%를, Python 3.10 cleanup/CLI는 109 passed,
  5 platform-only skipped, 20 subtests를 통과했다. 관련 XCB Qt 감사 18/18,
  Ruff, compileall, Action pin 14/14, diff check도 통과했다. 실제
  `outputs/`는 건드리지 않았으며 exact-head 원격 3-OS/CI는 아직 대기 중이다.
...
| 2 | `agent/safe-output-cleanup` / SAFE-01 | B1 | 로컬 후보 통과; exact-head 원격 3-OS/CI 후 병합 또는 command disabled 유지 |
| 3 | `agent/readme-contract-gates` / DOC-01 | SAFE-01 | KR/EN·명령·링크·repository-map PR-blocking 자동 gate |
...
2026-07-21 현재 Lane C의 GOV-01A/B/C는 완료됐다. SAFE-01은 clean
`main@41be887` 기반 로컬 후보와 자동 검증까지 준비됐지만 exact commit, PR, required
exact-head 6/6, 원격 3-OS 결과가 없으므로 완료로 판정하지 않는다. 실제 learner
outputs에는 테스트나 apply를 수행하지 않았다.
...
README/CLI 변경 때 D1/D2를 PR-blocking 자동화하는 작업은 DOC-01에서 추가한다.
...
- 동시에 진행 가능한 것: SAFE-01 remote gate와 signing/license/pilot의 외부 준비.
  DOC-01→COMP-01→COMP-02는 계약 순서대로 병합하며, 완료된 GOV-01은 설정 회귀 감시만
  유지한다.
...
1. scoped independent GO를 받은 clean `main@41be887` 기반 SAFE-01 후보의 최종 로컬
   gate를 재실행하고 exact commit을 만든다.
2. SAFE-01 PR에서 required exact-head 6/6과 Windows·Ubuntu·macOS desktop matrix를
   통과시킨다. 실제 `outputs/`에는 테스트나 apply를 수행하지 않는다.
3. review와 원격 gate가 모두 green일 때만 active ruleset을 통해 병합한다. merge 자체를
   SAFE-01 PASS로 간주하지 않는다.
4. 병합된 main의 post-merge required checks·3-OS desktop과 repository setting을
   재확인하고 merge SHA/URL을 baseline에 기록한 뒤에만 SAFE-01 PASS를 선언한다.
5. SAFE-01 PASS 뒤 실제 outputs root에서는 default dry-run만 생성한다. raw run name이나
   사용자 경로는 원격에 올리지 않고 inventory hash, plan ID, 집계, 제외 사유와 owner
   판단만 기록한다. owner 검토와 별개 명시적 승인 전에는 apply하지 않는다.
6. 별도 clean worktree에서 DOC-01 자동 gate를 먼저 추가한 뒤 COMP-01 contract matrix와
   COMP-02 소비자 변경을 각각 별도 PR로 진행한다.
7. GOV-01, SAFE-01, DOC-01, COMP-01/02가 모두 통과하기 전에는 B2를 선언하지 않는다.

**SAFE-01이 post-merge 검증까지 통과하기 전에는 `mclab clean`을 실행하지 않는다.
그 뒤에도 실제 outputs root의 quarantine apply는 owner가 같은 plan의 dry-run을
검토하고 별도 명시적으로 승인하기 전에는 금지한다.
GOV-01이 완료됐다는 이유만으로 SAFE-01 또는 B2를 완료 처리하지 않는다.**

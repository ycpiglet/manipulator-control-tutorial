...
- 상태: **Execution baseline v10 — COMP-02 PASS; BASE-01 candidate next, B2 owner acceptance pending**
- 작성일: 2026-07-22 KST
...
- COMP-02 handoff 시작 기준: corrective post-merge 6/6 뒤 새로 fetch한 clean
  `origin/main` containing `a2266b4f21f9a794998a98e71fa93643cacd1b64`
- BASE-01 시작 기준: COMP-02 corrective post-merge 6/6과 durable handoff 병합 뒤
  새로 fetch한 latest clean `origin/main`
...
| 6 | `agent/safe-main-baseline` / BASE-01 | 다음; durable COMP-02 handoff 병합 대기 | B2 후보 선언서와 exact-main evidence만 기록 |
...
BASE-01은 이 durable handoff 뒤 latest clean main에서 G1 전체 지표, live
governance, B2-scope blocker 0, review topology를 다시 집계하고 exact subject와 PR
head에 대한 owner 위험 수용을 받아야 한다. 그 전에는 B2를 선언하지 않는다.
...
- 이 durable handoff가 main에 도달한 뒤 동시에 진행 가능한 것: BASE-01의
  read-only evidence 수집과 signing/license/pilot의 내부 준비. 완료된 GOV-01,
  SAFE-01, DOC-01, COMP-01/02는 설정·안전·계약 회귀 감시만 유지하며, 다음 코드
  merge lane은 exact-main BASE-01 candidate다.
...
1. PASS로 갱신된 durable COMP-02 handoff를 protected main을 통해 병합한다.
2. 그 handoff까지 병합된 최신 `origin/main`을 fetch하고 새 clean
   `agent/safe-main-baseline` worktree를 만든다. 오래된 checkout이나 COMP-02
   worktree를 BASE subject로 재사용하지 않는다.
3. exact main commit/tree, `configs/`·`models/` inventory hash, workflow run/job,
   생성 시각, tool/OS/Python, threshold와 measurement를 BASE-01 evidence manifest에
   기록한다. dependency lock은 정확히 `N/A — SUP-01 pending`으로 쓴다.
4. active ruleset, security settings, direct collaborator 수, Action provenance와 G1
   전체 metric을 live read-only snapshot으로 다시 확인하고 B2-scope blocker 0을
   증명한다.
5. BASE-01 PR을 먼저 `CANDIDATE — owner acceptance pending`으로 만들고 독립
   read-only review와 exact-head six checks를 수행한다. 일반적인 작업 진행 지시나
   merge를 owner acceptance로 간주하지 않는다.
6. owner가 exact baseline subject와 PR head에 대해 single-maintainer 잔여 위험을
   명시적으로 수용한 뒤에만 B2 최종 선언을 판단한다. head가 바뀌면 다시 수용받는다.

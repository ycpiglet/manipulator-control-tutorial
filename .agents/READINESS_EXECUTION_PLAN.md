...
- 상태: **Execution baseline v8 — COMP-02 PR #53 open; merge/post-merge G1 evidence pending**
- 작성일: 2026-07-22 KST
...
- COMP-02 로컬 후보: `872ad7b7ce2949751873c47e7149300c4a521930`
- COMP-02 검증 functional head: `7e54f8cffad7b82408c6352a130a1a463013dfc8`
- 작업 시작 기준: clean fetched `origin/main@edaeb340765b076e761f5cd23ac588dda6729ba3`
...
- COMP-02는 clean fetched
  ...
  `872ad7b7ce2949751873c47e7149300c4a521930`으로 로컬 동결됐다. strict
  ...
  연결, ordered wall-preset 저장 E2E를 포함한다. 최종 Python 3.10 회귀는
  825 passed, 7 skipped, 2,422 subtests와 coverage 82.45%를 기록했고,
  78개 target의 292개 cross-surface case mismatch는 0이다. 독립 로컬 최종
  review는 P0/P1 없이 GO를 냈다. Draft PR #53 functional head `7e54f8c`는
  CI `29861932148`과 desktop matrix `29861932100`에서 required 6/6을
  통과했다. 이 상태-only successor head의 재검증, review conversation,
  merge, post-merge evidence가 남았으므로 COMP-02와 G1은 완료로 선언하지 않는다.
- COMP-02 이전 기준선 `origin/main@edaeb340`의 Desktop 완료 판정은 manifest의
  `status == "completed"`를 중심으로 세며 catalog/menu가 약속한 plot, control,
  prediction, observation 증거와 불일치한다. Functional head `7e54f8c`이 이
  불일치를 제거했지만 successor-head gate와 merge가 남아 있다.
...
| 5 | `agent/completion-consumers` / COMP-02 | Draft PR #53; successor-head gate 대기 | 모든 surface parity와 pinned evidence reader |
...
Lane B2의 COMP-02 implementation `872ad7b7`과 functional head `7e54f8c`은
로컬 G1 검증과 독립 review를 통과했고 draft PR #53 functional head도 required
6/6을 통과했다. 상태-only successor head 재검증, review conversation, merge,
post-merge evidence가 남았으므로 완료로 선언하지 않는다. 다음 merge lane은 계속
COMP-02다.
...
- 동시에 진행 가능한 것: COMP-02 구현과 signing/license/pilot의 외부 준비.
  완료된 DOC-01·COMP-01 뒤 COMP-02를 별도 PR로 병합하며, 완료된 GOV-01과
...
1. 이 state-only handoff update를 draft PR #53의
   `agent/completion-consumers` branch에 push한다.
2. resulting exact head에서 six required checks와 모든 review conversation을 통과시킨다.
   red check가 있으면 같은 branch에서 원인을 수정하고 새 exact head 전체를 다시
   검증한다.
3. green exact head만 protected main을 통해 merge하고 merge tree/source tree 동일성과
   post-merge six checks를 재확인한다. 그 전에는 COMP-02 PASS를 선언하지 않는다.
4. COMP-02 post-merge PASS 뒤 최신 main의 새 clean worktree에서 BASE-01 evidence
   manifest와 single-maintainer owner risk acceptance를 기록한다. 그 전에는 B2를
   선언하지 않는다.
5. 실제 outputs root dry-run은 COMP-02와 분리해 owner가 참여할 때만 기본 dry-run으로

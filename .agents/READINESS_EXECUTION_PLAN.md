...
- 상태: **Execution baseline v13 — B2/G1 declared; STATE-01 change ready; IA-00 follows acceptance**
- 작성일: 2026-07-22 KST
...
- STATE-01 change-set status: **content complete; acceptance is not inferred
  from this file**. Exact-head checks, protected merge, and post-merge checks
  remain the acceptance authority. Active handoff, immutable
...
| INT-01 | Feature owners | STATE-01 | M | stale packaging/font changes reviewed and reapplied as separate PRs, never wholesale copied |
...
| 7a | `agent/project-state-ledger` / STATE-01 | change ready; PR acceptance gate | concise live state, immutable history archive, append-only 19-finding ledger |
| 7b | `agent/repository-ia-decision` / IA-00 | **다음**; accepted STATE-01 뒤 merge | launcher no-move inventory와 beta 전 정리/동결 ADR |
| 7c | `agent/launcher-compatibility` / IA-01 | IA-00 GO | PKG/E2E 전에 shim 우선 정리하거나 v0.1 경로 동결 |
| 8a | Draft PR #38 / INT-01 font | STATE-01, owner intent·Windows 실기기 확인 | 최소 hunk 채택 또는 superseded로 종료 |
| 8b | Draft PR #37 / INT-01 package | STATE-01 | 구조 검증과 baseline delta 후 채택 또는 종료 |
| 9 | `agent/dependency-reproducibility` / SUP-01 | STATE-01 | lock, pinned asset/action, scans |
| 10 | `agent/third-party-notices` / LIC-01 | SUP-01 inventory merge | notices와 compliance 승인 |
| 11 | `agent/package-release-metrics` / PKG-01 | IA-00 결정, IA-01 (GO 시), INT-01 결정 | size/startup contract |
...
각 행의 선행 조건을 충족한 뒤, 7b의 read-only draft만 7a와 병렬로 준비할 수 있고
7b merge는 accepted STATE-01 뒤에만 가능하다. 8a와 8b, 9와 11, 13과 14는 파일
ownership이 겹치지 않으면 병렬로 진행할 수 있다. 표의 순서는 위험과 최종 merge
의존성을 뜻하며 모든 작업을 직렬화하라는 의미는 아니다.
...
### Wave 2 — 깨끗한 안전 기준선과 보류 변경 재적용 — B2 완료, STATE change ready

1. 구현 완료: `9eb8eb19`에서 clean worktree를 만들고 STATE-01 범위를 격리했다.
2. 구현 완료: B2 뒤 첫 merge lane인 STATE-01에서 live handoff, history archive,
   finding-status ledger를 분리했다. Exact-head, protected merge, post-merge evidence가
   acceptance를 결정하며, 그 뒤 다음 merge는 IA-00 결정 record다.
3. 감사 당시 dirty checkout의 package-size 묶음은 Draft PR #37, 별도 복구된 font
   묶음은 Draft PR #38에 원격 보존됐다. 둘 다 B2 전에는 병합하지 않는다.
4. PR #38은 오래된 `qt_app.py` 전체 대신 현재 main에 필요한 hunk만 forward-port했다.
5. 다음: IA-00에서 launcher를 beta 전 정리할지 v0.1 public path로 동결할지
   결정한다. Read-only inventory와 독립 교차검토는 B4까지 기존 경로 동결을
   권고한다. ADR이 이를 채택하면 IA-01 없이 PKG/INT prerequisite로 진행한다.
6. STATE-01 뒤 두 Draft PR을 각각 rebase하고 구조·패키지·실기기 증거로 채택 또는
   종료한다. Wholesale merge는 금지한다.
7. Exact SHA, config/model hashes, 기본 CI와
   `dependency lock: N/A — SUP-01 pending`은 B2 declaration에 기록됐다.
...
1. STATE-01 뒤 `agent/dependency-reproducibility` — SUP-01
2. SUP-01 inventory merge 뒤 `agent/third-party-notices` — LIC-01
3. IA-00/INT-01 결정과 조건부 IA-01 뒤
   `agent/package-release-metrics` — PKG-01
4. SUP-01과 PKG-01 merge 뒤 `agent/packaged-e2e` — E2E-01
...
SUP-01과 PKG-01 변화가 모두 병합된 exact commit에서 다시 수행한다. Unsigned
artifact는 trusted invited preview로만 사용하고 7일 CI artifact 만료 전에 검증
증거를 별도로 보존한다.
...
- B2 뒤 첫 merge lane인 STATE-01 change set은 준비됐다. Exact-head checks,
  protected merge, post-merge checks가 acceptance를 증명한 뒤 다음 merge lane은
  rebase한 IA-00 no-move ADR이다. SUP-01과 disjoint INT/OPS/EDU/MAINT lane도 각 prerequisite를
  지키며 시작할 수 있다. PKG-01은 IA-00/INT-01 결정과 조건부 IA-01 뒤에만
  시작하며 workflow/packaging 최종 변경은 순차 merge한다. 완료된 GOV-01,
  SAFE-01, DOC-01, COMP-01/02,
  BASE-01은 설정·안전·계약 회귀 감시만 유지한다.
...
1. STATE-01 change set은 구현됐다. Exact-head, protected merge, post-merge evidence로
   acceptance를 확인한 뒤 clean `agent/repository-ia-decision` worktree에서 IA-00
   no-move ADR만 병합한다. 검토된 권고는 현재 root launcher 23개와 package/CLI
   entrypoint를 v0.1/B4까지 동결하고 IA-01을 만들지 않는 것이다. ADR은 inventory,
   consumer/test gap, rollback과 재검토 trigger를 보존한다.
2. IA-00이 예상과 달리 launcher 정리를 채택할 때만 별도 IA-01 compatibility/shim
   PR을 PKG/E2E보다 먼저 수행한다. Config/model/paper 경로는 이동하지 않는다.
3. Draft PR #37/#38은 B1 기반 묶음이므로 wholesale merge하지 않는다. 최신 main에
   필요한 hunk만 각각 다시 검토하며, #38은 owner intent와 실제 Windows 한국어 glyph
   확인, #37은 structural exclusion test와 같은 workflow의 size delta가 필요하다.
4. SUP-01을 첫 technical baseline lane으로 수행하고 lock/constraints, simulator-CI
   Menagerie pin, vulnerability/license scans, SBOM input을 고정한다. LIC-01은 그
   inventory가 병합된 뒤 수행한다. PKG-01은 IA/INT 결정과 조건부 IA-01을 반영하고,
   E2E-01은 SUP-01과 PKG-01이 병합된 최종 commit에서 수행한다.
...
7. B2나 STATE-01을 public-beta, signed distribution, release/DOI, 실제 cleanup,
   external contact, 구조 변경 승인으로 해석하지 않는다. G2부터 G5까지의 외부
   결정과 실기기 gate를 유지한다.

**SAFE-01은 PASS지만 실제 outputs root dry-run은 현재 승인되지 않았다. 향후
owner가 참여해 만든 exact plan도 owner가 검토하고 다시 명시적으로 승인하기 전에는
quarantine apply가 금지된다. B2나 STATE-01을 실제 cleanup, 공개 배포, signing,
release/DOI 승인으로 해석하지 않는다.**

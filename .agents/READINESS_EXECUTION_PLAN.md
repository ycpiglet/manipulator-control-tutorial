...
- 상태: **Execution baseline v12 — B2 safe-main DECLARED; G1 12/12 PASS**
- 작성일: 2026-07-22 KST
...
| 7b | `agent/repository-ia-decision` / IA-00 | B2에서 draft, STATE-01 뒤 merge | launcher no-move inventory와 beta 전 정리/동결 ADR |
...
각 행의 선행 조건을 충족한 뒤, 7a와 7b, 8a와 8b, 9와 11, 13과 14는 파일
...
### Wave 2 — 깨끗한 안전 기준선과 보류 변경 재적용 — B2 완료, 후속 진행

1. 최신 main에서 clean worktree를 만들고 status/diff/upstream을 기록한다.
2. B2 뒤 첫 merge로 STATE-01을 수행한다. IA-00은 병렬 draft할 수 있지만 결정
   record는 STATE-01 뒤 병합한다.
...
5. IA-00에서 launcher를 beta 전 정리할지 v0.1 public path로 동결할지 결정한다.
...
- B2 선언 직후 첫 merge lane은 STATE-01이다. IA-00은 no-move inventory/ADR로
  병렬 draft할 수 있지만 같은 handoff 파일을 만지면 STATE-01 merge 뒤 rebase한다.
  STATE-01 merge 뒤 SUP-01과 disjoint INT/OPS/EDU/MAINT lane을 시작할 수 있다.
  PKG-01은 IA-00/INT-01 결정과 조건부 IA-01 뒤에만 시작하며 workflow/packaging
  최종 변경은 순차 merge한다. 완료된 GOV-01, SAFE-01, DOC-01, COMP-01/02,
...
1. 최신 `origin/main`에서 clean `agent/project-state-ledger` worktree를 만들고
   STATE-01만 수행한다. `CURRENT_STATE.md`의 오래된 이력을 archive로 이동하고
   감사 finding-status ledger를 append-only로 만든다. Audit source는 수정하지 않는다.
2. IA-00은 병렬 draft할 수 있지만 no-move inventory와 launcher ADR만 포함한다.
   STATE-01과 파일이 겹치면 STATE를 먼저 병합하고 IA를 rebase한다.
3. IA-00이 launcher 정리를 채택하면 별도 IA-01 compatibility/shim PR을 PKG/E2E보다
   먼저 수행한다. 미채택이면 현재 launcher 경로를 v0.1/B4까지 동결한다.
   Config/model/paper 경로는 이동하지 않는다.
4. Draft PR #37/#38은 B1 기반 묶음이므로 wholesale merge하지 않는다. 최신 main에
...
5. SUP-01을 첫 technical baseline lane으로 수행하고 lock/constraints, simulator-CI
...
6. 실제 outputs root dry-run은 B2와 분리해 owner가 참여할 때만 기본 dry-run으로
   수행한다. raw run name이나 사용자 경로는 원격에 올리지 않고 inventory hash,
   plan ID, 집계, 제외 사유와 owner 판단만 기록한다.
7. B2를 public-beta, signed distribution, release/DOI, 실제 cleanup, 구조 변경
   승인으로 해석하지 않는다. G2부터 G5까지의 외부 결정과 실기기 gate를 유지한다.

**SAFE-01은 PASS지만 실제 outputs root의 quarantine apply는 owner가 같은 plan의
dry-run을 검토하고 별도 명시적으로 승인하기 전에는 금지한다. B2 선언도 실제
cleanup, 공개 배포, signing, release/DOI 승인으로 해석하지 않는다.**

...
- 상태: **Execution baseline v2 — B1 선언, Wave 1 진행 중**
- 작성일: 2026-07-20 KST
- 현재 기준 원격: `origin/main@f04cca16848316e227df18fe129229b515ae01c7`
...
1. PR #35로 README, 문서 진입점, 감사 기록, 인수인계 기준선을 병합하고 B1을
   선언했다.
2. 저장소 정책·보호 설정·Actions 고정을 GOV-01A/B/C로 나누어 완료한다.
3. 데이터 손실 위험이 있는 `mclab clean`과 잘못된 학습 완료 판정을 별도의 P0 PR로
   수정한다.
...
- B1 선언 시점 GitHub main에는 branch protection/ruleset이 없고, vulnerability
  alerts, dependency lock, immutable release, tag가 준비되지 않았다. 이 문서에서
  완료로 기록하려면 실제 설정 API 재조회 증거가 필요하다.
...
| 1a | `agent/governance-baseline` / GOV-01A | B1 | policy files; GOV-01 전체는 아직 미완료 |
| 1b | repository settings / GOV-01B | GOV-01A 병합, owner 권한 | ruleset와 security settings API snapshot |
| 1c | `agent/pin-actions` / GOV-01C | GOV-01B | Actions full-SHA pinning과 green replacement runs |
...
| 6a | Draft PR #38 / INT-01 font | B2, owner intent·Windows 실기기 확인 | 최소 hunk 채택 또는 superseded로 종료 |
| 6b | Draft PR #37 / INT-01 package | B2 | 구조 검증과 baseline delta 후 채택 또는 종료 |
...
SAFE-01, COMP-01, GOV-01A/C는 파일상 독립 PR로 개발할 수 있다. 권장 merge 순서는
GOV-01A, GOV-01B 설정 검증, GOV-01C, SAFE-01, COMP-01, COMP-02다. 저장소 setting에
...
4. B2 이후 두 Draft PR을 각각 rebase하고 구조·패키지·실기기 증거로 채택 또는 종료한다.
...
1. GOV-01A 정책 PR을 exact-head CI와 owner review 후 병합한다.
2. private vulnerability reporting을 포함한 GOV-01B 설정을 적용하고 API로 다시 읽어
   증거를 보존한다.
3. GOV-01C에서 workflow의 third-party Actions를 reviewed full commit SHA로 고정하고
   replacement runs가 모두 green인지 확인한다.
4. 최신 main clean worktree에서 SAFE-01 실패 fixture부터 작성하고 실제 `outputs/`에는
   손대지 않는다.
5. 별도 clean worktree에서 COMP-01 contract matrix를 준비하되 COMP-02 소비자 변경과
   섞지 않는다.
6. GOV-01A/B/C, SAFE-01, COMP-01/02가 모두 통과하기 전에는 B2를 선언하지 않는다.

**SAFE-01이 G1을 통과하기 전에는 `mclab clean`을 실행하지 않는다. GOV-01A 정책 파일만
병합됐거나 설정만 적용됐다는 이유로 GOV-01 또는 B2를 완료 처리하지 않는다.**

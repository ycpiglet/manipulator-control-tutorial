...
- 상태: **Execution baseline v11 — BASE-01 G1 metrics 11/12; candidate
  exact-head evidence and owner acceptance pending**
- 작성일: 2026-07-22 KST
...
- BASE-01 frozen subject/tree:
  `ae4d40371b9a3fe4d7078822e1fc541a72defe2d` /
  `b5f4cb03abbf1e66c14c27c720c737ef2fc4f798`
- BASE-01 evidence status: G1 metrics **11/12 PASS**; review topology is the
  sole pending metric row. Candidate exact-head checks, independent review,
  and exact-subject/exact-head owner acceptance remain separate BASE-01
  workflow prerequisites at record creation.
...
| 6 | `agent/safe-main-baseline` / BASE-01 | 진행 중; G1 metrics 11/12, final-head prerequisites pending | exact-subject 후보 선언서, 독립 review, exact-head 6/6, 명시적 owner acceptance |
...
재집계했으며 현재 11/12 PASS다. Candidate PR의 독립 review와 exact-head checks를
끝낸 뒤 exact subject와 PR head에 대한 owner 위험 수용을 받아야 한다. 그 전에는
B2를 선언하지 않는다.

완료 결과: corrective post-merge 6/6과 review conversation resolution PASS.
...
- 현재 유일한 active merge lane은 BASE-01 candidate 기록, 독립 read-only review,
  exact-head six checks, exact-SHA owner acceptance다. Signing/license/pilot 내부
  준비는 병렬로 할 수 있지만
  B2 이후 작업을 병합하거나 구조를 변경하지 않는다. 완료된 GOV-01, SAFE-01,
  DOC-01, COMP-01/02는 설정·안전·계약 회귀 감시만 유지한다.
...
1. Frozen subject `ae4d40371b9a3fe4d7078822e1fc541a72defe2d`와 tree
   `b5f4cb03abbf1e66c14c27c720c737ef2fc4f798`의 collected evidence를
   `.agents/baselines/B2-safe-main.md`와 machine-readable companion에
   `CANDIDATE — checks, review, and owner acceptance pending at record creation`으로
   기록한다.
2. G1 scorecard의 12개 metric 중 유일한 미충족 row가 review topology이고 이를
   닫으려면 exact-subject/exact-PR-head owner acceptance가 필요함을 명시한다. 이
   11/12 metric 집계는 BASE-01 workflow의 남은 prerequisite 수가 아니다. Candidate
   exact-head six checks, independent read-only review, exact-SHA owner acceptance를
   같은 final head에서 각각 확인해야 한다. Dependency lock은
   `N/A — SUP-01 pending`으로 기록하고 deferred
   supply-chain, notice, signing/package, producer-pin 위험을 숨기지 않는다.
3. BASE-01 candidate PR을 열고 exact head를 고정한 뒤 독립 read-only review와
   required six checks를 수행한다.
4. Review finding이나 head 변경이 있으면 보수하고 review/check를 다시 수행한다.
5. Owner가 frozen subject와 최종 PR head를 함께 명시하여 single-maintainer 잔여
   위험을 수용한 뒤에만 B2 최종 선언을 판단한다. 일반 작업 진행 지시나 merge
   권한은 acceptance가 아니다.
6. Owner acceptance 전에 BASE-01을 PASS/B2로 표시하거나 B2 후속 merge lane을
   시작하지 않는다.
7. 실제 outputs root dry-run은 BASE-01과 분리해 owner가 참여할 때만 기본 dry-run으로

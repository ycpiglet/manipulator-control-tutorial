...
- CLI `python -m mclab coverage` prints the saved Experience Coverage summary, seven-item coverage map, next missing experience, mode, action, evidence cue, ready-to-run command, and compact next-experience guide with try/change/value/prediction/question/watch cues for VS Code terminal-only learners

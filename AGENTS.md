...
- Lab03 condition-aware DLS fixed/adaptive speed-retarget comparison configs for adaptive target-speed scheduling lessons
- Lab03 retargeting DLS viewer guides show small green waypoint path markers for target-path lessons
- Learner menu exposes Lab03 upper/lower path, shoulder/elbow disturbance, low/high torque-limit, slow/fast command-speed, low/high joint-speed-limit, direct/inward retargeting, and fixed/adaptive speed-retarget DLS scenario cards with compare/search/filter support
...
- Batch reports and worksheets include a `Viewer Handoff` recommendation that picks a scenario to reopen in the side-panel-free viewer based on the largest baseline metric change, with report, priority-plot, worksheet, folder, and course-level anchor links
...
- MuJoCo viewer side panels are hidden at launch and re-hidden during viewer sync for learner demos; use the `MCLab Interaction` panel and YAML configs as the control surface
...
- Top-level CLI `--help` prints a learner workflow for setup checking, menu launch, coverage, parameter lookup, next preview, next path-step launch, review, and outputs index opening
...
- CLI `python -m mclab` and `python -m mclab list` print learner entry points for doctor, menu, coverage, coverage details, parameter lookup, next preview, launching the next path step, review, and opening the outputs index before the low-level lab/batch names
- CLI run and batch completion output lists key artifact paths such as report, worksheet, the cumulative outputs index, plots, comparison plots, plus the next experience, mode, action, evidence cue, and command when course coverage is available
...
- CLI `python -m mclab scenarios <terms> --filter <type>` searches guided learner scenarios using the same terms as the GUI menu and prints discovery examples, mission, start steps, challenge, controls, and ready-to-run commands; `--details` also prints playbook, try/change/value/prediction/question/watch cues, viewer legend, readiness, and control-credit cues
- CLI `scenarios`, `params`, and `batches` discovery outputs end with `python -m mclab review` and `python -m mclab index --open` follow-up commands so terminal-only learners continue from finding a run to reviewing saved evidence and reports
...
- CLI `python -m mclab coverage` prints the saved Experience Coverage summary, seven-item coverage map, next missing experience, mode, action, evidence cue, ready-to-run command, and compact next-experience guide with try/change/value/prediction/question/watch cues for VS Code terminal-only learners; when saved hands-on evidence still needs observation, required-preset, or control repair it also prints an `Evidence repair queue` handoff to `python -m mclab review --limit 3` plus the top repair target; when all seven experience types are covered it points to the recommended learning path summary, milestone status, next path step, command, and guide if path evidence is still pending; `--details` expands all seven experience types with mode, focus, evidence, command, controls, learner-control credit lines, and per-item `Review repair` rerun cues when a representative or related scenario still has saved-run evidence gaps
- CLI `python -m mclab path` prints the recommended 12-step learning-path summary, milestone status, next step, ready-to-run command, and compact next-step guide with try/change/value/prediction/question/watch cues; `--all` adds a one-line status map for every path step
- CLI `python -m mclab next --preview` prints the next recommended path step plus a compact Plan/Course/Mission/Try/Change/Values/Prediction/Question/Watch/Start steps/Challenge/Controls guide without running it, adding the Viewer guide only for hands-on steps and worksheet/plot/plot-review cues for batch steps; `python -m mclab next` prints the same guide, launches automatic steps headless and hands-on steps in the side-panel-free viewer, and opens the resulting report
- CLI `python -m mclab review` prints the saved-run review queue, next pending evidence status, report, worksheet, matched action, repair command, observation next step, plot review cue, a deduplicated top learner-action repair list controlled by `--limit` (`0` hides it) that folds older same-scenario saved runs into `+N older saved runs`, the separate recommended course-path next step/command, and a `Review index command` for reopening the full browser review table; required-preset gaps are named before generic observation gaps so wall learners see the exact next preset such as `Close wall`; `--open` opens the next pending report
- CLI `python -m mclab index --open` regenerates and opens `outputs/index.html` without launching the learner menu
- Learner-facing output discovery excludes internal verification folders whose names start with `_`, `codex_`, or `verify_` from review queues, latest-output buttons, coverage/path progress, and `outputs/index.html` so smoke artifacts do not count as learner evidence
- Outputs index includes `Starter Commands` for setup checking, opening the learner menu, checking coverage, comparing all coverage details, inspecting editable parameters, finding guided hands-on scenarios, finding comparison batches, previewing the next path step, reviewing saved evidence, opening all reports, generating first headless artifacts, launching the first hands-on viewer demo, and running the first comparison batch
- Outputs index includes the shared `Experience Coverage` summary plus Done/Next/Missing cards with mode, focus, evidence, controls, learner-control credit, CLI commands, and related `Review repair` rerun cues for all core experience types so browser-based review shows the same core experience evidence, next missing experience, and saved-run repair guidance as the learner menu and CLI
- Run reports and generated `worksheet.md` artifacts repeat related `Review repair` rerun cues inside `Course Experience Coverage` so a learner can open one saved report and still find the next observation, preset, or control repair
...
- Interactive `MCLab Interaction` live status starts with `Run time` and `Remaining` for every lab so learners know how much observation time is left
- Interactive `MCLab Interaction` live status shows a final-second `Run clock` cue that tells learners to pause if they need more inspection time
...
- Interactive `MCLab Interaction` panel shows a config-based `Course step` cue so the live viewer preserves the same recommended-path context as the learner menu and CLI
...
- Interactive `MCLab Interaction` panel `Done when` cue names required Quick preset order directly when completion evidence requires a preset path
...
- Interactive `MCLab Interaction` observation area shows a persistent `Saved observation` recap after `Mark observation` so learners can confirm prediction, outcome, note, control, and preset evidence plus the next review or repair step using concrete available control names
...
- Evidence helper buttons such as `Use live status`, `Use changed values`, and `Clear note`, plus view/pacing controls such as pause, step, and playback speed, do not satisfy the learner-control requirement by themselves
...
- Live panel `Start steps` uses actual panel button labels and control families when no required preset order overrides the first hands-on action
...
- Learner menu, CLI `next --preview`, and outputs index Learning Path cards show first-run hands-on `Next cue` text that names the prediction, viewer launch, first configured preset/button/slider action, and `Mark observation` step before any saved run exists
...
- Learner menu and CLI setup diagnostics through `python -m mclab doctor`, including next learner commands after successful checks and a menu status cue to use Run next or preview the path
...
- `python -m mclab doctor` checks learner menu scenario and comparison-batch readiness and prints launcher, coverage, coverage-details, preview, next-run, review, and outputs-index commands when setup is ready
...
- Run reports and worksheets include `Course Experience Coverage` with Done/Next/Missing status, mode, focus, evidence cues, and the next-experience command so learners can continue the varied course path from the report or worksheet that opens after each run
...
- Outputs index learning path cards show `Plan` cues for level, mode, duration, and saved artifacts before completion criteria
...
- fixed/adaptive speed-retarget condition-aware DLS comparison configs for Lab03
...
- richer condition-aware task-control comparisons, such as combined adaptive speed and disturbance schedules under changing targets

...
- CLI `python -m mclab review` prints the saved-run review queue, next pending evidence status, report, worksheet, matched action, observation next step, and plot review cue; required-preset gaps are named before generic observation gaps so wall learners see the exact next preset such as `Close wall`; `--open` opens the next pending report

...
- Learner-facing output discovery excludes internal verification folders whose names start with `_` or `codex_` from review queues, latest-output buttons, coverage/path progress, and `outputs/index.html` so smoke artifacts do not count as learner evidence

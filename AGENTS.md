...
- CLI `python -m mclab coverage` prints the saved Experience Coverage summary, seven-item coverage map, next missing experience, mode, action, evidence cue, ready-to-run command, and compact next-experience guide with try/change/value/prediction/question/watch cues for VS Code terminal-only learners; when all seven experience types are covered it points to the recommended learning path summary, milestone status, next path step, command, and guide if path evidence is still pending; `--details` expands all seven experience types with mode, focus, evidence, command, controls, and learner-control credit lines

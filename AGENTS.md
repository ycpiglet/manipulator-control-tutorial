...
- Learner menu includes an `Experience coverage` summary that tracks Intro basics, Hands-on controls, Comparison batch, 2DOF/Jacobian, Singularity/DLS, Panda manipulator, and Virtual wall saved evidence and exposes a `Run next` button for the next missing experience

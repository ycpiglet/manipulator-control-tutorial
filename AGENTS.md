...
- CLI `python -m mclab scenarios <terms> --details` also prints latest report/folder, history, mission/challenge evidence, observation/latest evidence, observation flow/next step, preset/activity mix, next cue, plot, worksheet, replay status, and replay command for terminal-only learners

...
Use the small `MCLab Interaction` window next to the viewer. Move `Target X`, `Target Y`, `Task stiffness`, `Task damping`, and `Torque limit` while the simulation is running, and use `Reset sliders` to return to the starting values. Press `Mark observation` when a response is worth comparing later. Use `Live status` to read `q1`, `q2`, hand X/Y, error norm, and max torque.

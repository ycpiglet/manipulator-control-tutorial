...
- `outputs/index.html`: 추천 학습 경로 진행표, hands-on evidence 상태, run별 observation/prediction/note 요약, 최신 prediction outcome 요약, 단계별 실행/반복 명령, Lab별 진행 요약, 실행 리포트, replayable tuned-config 바로가기, plot 바로가기, `Lesson` / `Next` 안내, 핵심 summary metric을 최신순으로 비교하는 목록 페이지
...
- `outputs/index.html`: a newest-first index with the recommended learning path, hands-on evidence status, per-run observation/prediction/note summaries, latest prediction-outcome snippets, per-step run/repeat commands, lab progress cards, run reports, replayable tuned-config shortcuts, plot shortcuts, `Lesson` / `Next` guidance, and key summary metrics

...
After each run or batch, the CLI prints the key artifact paths to open next, including `report.html`, `worksheet.md`, `outputs/index.html`, `plots/`, or `comparison_plots/` when they exist. When the worksheet includes course coverage, it also prints the next experience and ready-to-run next command.

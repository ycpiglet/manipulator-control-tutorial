...
실행 중에도 같은 `Mission`, `Playbook`, `Viewer legend`, `Done when` 완료 기준이 패널 상단에 표시되므로, MuJoCo 기본 사이드 패널 없이도 화면의 색상 마커와 학습 증거 조건을 바로 해석할 수 있습니다.
...
The same `Mission`, `Playbook`, `Viewer legend`, and `Done when` completion criterion appear near the top of the panel during a live run, so learners can read the colored scene markers and evidence requirement without opening the MuJoCo side panels.

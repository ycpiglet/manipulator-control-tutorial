...
`Experience coverage`는 메뉴와 `outputs/index.html`에서 저장된 실행 기록을 읽어 Intro basics, Hands-on controls, Comparison batch, 2DOF/Jacobian, Singularity/DLS, Panda manipulator, Virtual wall 체험군 중 무엇을 이미 해봤는지 보여줍니다. interactive run은 실제 버튼, 슬라이더, preset 중 하나를 사용해야 hands-on 체험으로 인정되며, 메뉴는 7개 체험군의 `Coverage map`과 아직 비어 있는 다음 체험군을 바로 실행하는 `Run next` 버튼을 보여줍니다. CLI만 사용할 때는 `python -m mclab coverage`로 같은 coverage map, 다음 체험의 mode/action/evidence cue, 다음 실행 명령, Plan/Mission/Try/Change/Values/Prediction/Question/Watch/Start steps/Challenge/Controls guide를 터미널에서 확인할 수 있습니다. 저장된 hands-on 실행에 observation, required preset, control 같은 증거 보완이 남아 있으면 같은 출력 끝에 `Evidence repair queue`와 첫 repair target이 표시되어 `python -m mclab review --limit 3`로 바로 이어갈 수 있습니다. 7개 체험군이 모두 채워졌지만 recommended learning path가 남아 있으면 같은 명령이 path summary, milestone, next path step, command, guide를 이어서 출력합니다. `python -m mclab coverage --details`는 7개 체험군 전체의 mode, focus, evidence, command, controls, learner-control credit을 펼쳐 보여주고, 해당 체험군의 대표 또는 관련 scenario에 saved-run 보완이 남아 있으면 `Review repair` 줄로 필요한 preset/observation/control 보완과 rerun command를 바로 붙입니다. `outputs/index.html`도 7개 체험군별 Done/Next/Missing 카드에 Mode/Focus/Evidence/Controls/Counts-as-control, 각 체험을 실행할 CLI 명령, 관련 `Review repair` rerun cue를 함께 보여주며, 개별 `report.html`과 `worksheet.md`의 `Course Experience Coverage`도 같은 관련 repair cue를 표시해 리포트 하나만 열어도 다음 보완 실행으로 이어지게 합니다. coverage가 완료되면 아래 Learning Path 섹션을 계속 보라고 안내합니다.
...
`Experience coverage` appears in the menu and `outputs/index.html`. It reads saved outputs and shows which core experience types have evidence: Intro basics, Hands-on controls, Comparison batch, 2DOF/Jacobian, Singularity/DLS, Panda manipulator, and Virtual wall. Interactive runs count as hands-on only after the learner uses at least one button, slider, or preset, the menu shows a seven-item `Coverage map` plus a `Run next` button for the next missing experience, CLI-only learners can run `python -m mclab coverage` to print the same map, next-experience mode/action/evidence cue, next command, and next-experience Plan/Mission/Try/Change/Values/Prediction/Question/Watch/Start steps/Challenge/Controls guide in the terminal, saved hands-on runs with remaining observation, required-preset, or control evidence now add an `Evidence repair queue` line with the first repair target and a `python -m mclab review --limit 3` handoff, the same command prints the recommended-path summary, milestone status, next path step, command, and guide when all seven core experiences are covered but path evidence is still pending, `python -m mclab coverage --details` expands all seven experience types with mode, focus, evidence, command, controls, and learner-control credit lines and adds a per-item `Review repair` line when that representative or related scenario still needs a saved-run evidence fix, `outputs/index.html` shows Done/Next/Missing cards with Mode/Focus/Evidence/Controls/Counts-as-control, matching CLI commands, and related `Review repair` rerun cues, and each run's `report.html` plus `worksheet.md` now repeats those related repair cues in `Course Experience Coverage` so one opened report can still hand the learner to the next fix. Completed coverage points back to the Learning Path section.
...
In `python -m mclab review`, `Next review` means the saved run whose evidence should be repaired first, `Repair command` is the rerun command for fixing that evidence, and `Course path next` means the next recommended learning-path step to run. Use `Next review` plus `Repair command` for cleanup and `Course path command` when continuing the course. The same output's `Review index command` reopens the browser view for the full review queue, run table, and report/worksheet/plot links. `python -m mclab review --limit 3` also prints up to N deduplicated learner-action repair targets, folding older saved runs for the same scenario into `+N older saved runs`; `--limit 0` hides that list for shorter classroom output. Learner review queues, latest-output buttons, coverage/path progress, and `outputs/index.html` exclude internal verification output folders whose names start with `_`, `codex_`, or `verify_`, so smoke or verification artifacts do not count as learner evidence. The `Repair command` and `Repeat command` snippets in `outputs/index.html` preserve YAML or learning-path plot presets, so wall, DLS, and Cartesian review runs keep their priority graphs.
...
Running `python -m mclab` with no subcommand prints the same entry points for `doctor`, `menu`, `coverage`, `coverage --details`, `params`, `next --preview`, `next`, `review`, and `index --open`.

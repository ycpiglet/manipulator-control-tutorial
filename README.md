...
Running `python -m mclab` with no subcommand prints the same entry points for `doctor`, `menu`, `coverage`, `coverage --details`, `params`, `next --preview`, and `next`.

...
CLI discovery 명령은 결과가 있을 때도 다음 검색 예시를 함께 보여줍니다. `scenarios`는 hands-on wall, singularity, prediction/live-target 예시를 안내하고, `batches`는 wall, 2DOF, all batch 예시를 안내합니다.
...
CLI discovery commands now print example searches even when matches are found. `scenarios` suggests hands-on wall, singularity, and prediction/live-target searches, while `batches` suggests wall, 2DOF, and all-batch searches.

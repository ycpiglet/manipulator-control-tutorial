...
- `report.html`: `Try` / `Change` / `Prediction` / `Question` / `Watch` 학습 가이드, run-level `Done when` 완료 기준, 바로 다음에 할 일을 묶은 `Next Actions`, 재현 실행 명령, replay tuned config 명령, 다음 실험 prediction/question/key parameter/control surface 요약이 포함된 suggested next runs, 관련 comparison batch 명령, 현재 실행의 버튼/슬라이더/preset/evidence control을 보여주는 `Control Surface`, config highlights, configured preset cards, singularity/DLS/wall/effort를 포함한 자동 결과 점검, hands-on evidence 완료 상태, learner action summary, learner snapshot, preset choices와 preset comparison progress, prediction review, prediction-observation pair review cue, prediction outcome, prediction/evidence/note가 포함된 observation marker review prompt, 요약값, notes, plot 이미지와 plot 해석 가이드를 한 화면에서 보는 실행 리포트
- `worksheet.md`: `Done when` 완료 기준, 예측, prediction outcome, pending outcome review, learner note, live status, 핵심 파라미터, priority plot review, review checklist, suggested next experiment, comparison batch 명령을 Markdown으로 정리한 복습/제출용 워크시트
- `outputs/index.html`: 추천 학습 경로 진행표, 단계별 `Done when` 완료 기준, evidence/outcome pending 상단 요약, hands-on evidence 상태, 단계별 prediction/compare/watch cue, 최신 evidence 요약과 report/worksheet/priority plot/replay 바로가기, evidence quality와 outcome mix 요약, run별 observation/prediction/outcome/note 요약, 최신 prediction outcome 요약, 단계별 실행/반복 명령, Lab별 진행 요약, 실행 리포트, worksheet, replayable tuned-config 바로가기, plot 바로가기, `Lesson` / `Next` 안내, 핵심 summary metric을 최신순으로 비교하는 목록 페이지

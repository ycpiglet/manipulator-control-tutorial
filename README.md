...
`Saved observation` 줄은 마지막으로 저장한 observation marker의 prediction, outcome, note item 수, learner-control evidence, preset comparison 저장 여부를 계속 보여주므로, 메모 칸이 비워진 뒤에도 방금 남긴 증거를 확인할 수 있습니다.
...
The `Saved observation` line keeps the latest marker recap visible after saving, including prediction, outcome, note-item count, learner-control evidence, and preset-comparison evidence, so learners can confirm what was recorded even after the note box clears.

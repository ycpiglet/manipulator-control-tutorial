...
`python -m mclab review`의 `Next review`는 저장된 실행 중 먼저 보완할 증거를 뜻하고, `Course path next`는 추천 학습 경로에서 다음에 실행할 단계를 뜻합니다. 두 줄이 다를 수 있으므로 복습할 때는 `Next review`를, 수업을 계속 진행할 때는 `Course path command`를 사용합니다.
...
In `python -m mclab review`, `Next review` means the saved run whose evidence should be repaired first, while `Course path next` means the next recommended learning-path step to run. These can differ, so use `Next review` for cleanup and `Course path command` when continuing the course.

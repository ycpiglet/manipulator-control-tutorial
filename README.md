...
- `report.html`: `Try` / `Change` / `Watch` 학습 가이드, 재현 실행 명령, config highlights, singularity/DLS/wall/effort를 포함한 자동 결과 점검, observation marker, 요약값, notes, plot 이미지와 plot 해석 가이드를 한 화면에서 보는 실행 리포트
...
- `report.html`: a one-page run report with `Try` / `Change` / `Watch` learning guidance, reproduce commands, config highlights, automatic result checks for signals such as singularity, DLS speed, wall response, and actuator effort, observation markers, summary values, notes, plot images, and plot interpretation guides

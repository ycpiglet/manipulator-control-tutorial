...
BATCH_ACTIONS: tuple[BatchMenuAction, ...] = (
    ...
    BatchMenuAction(
        ...
        description="Run joint-space, task-space, singularity, DLS, retarget, speed-limit, torque-limit, and disturbance cases.",
        try_this="Compare joint error, hand error, torque, manipulability, DLS damping, speed limits, and retarget paths.",
        watch="How task-space control, singularity handling, DLS schedules, and retarget strategy change the arm response.",
    ),
    BatchMenuAction(
        group="Comparison Batches",
...
LEARNING_PATH: tuple[LearningPathStep, ...] = (
    ...
    LearningPathStep(
        title="8. Compare DLS retarget",
        action_kind="run",
        group="Lab03 2DOF Arm and Trajectories",
        label="2DOF adaptive-speed retarget DLS",
        description="Compare a near-edge retarget path with an adaptive task-speed schedule before moving to Panda.",
    ),
    LearningPathStep(
        title="9. Hold Panda",
        action_kind="run",
...
MENU_ACTIONS: tuple[MenuAction, ...] = (
    ...
    MenuAction(
        group="Lab01 Mass-Spring-Damper",
        label="Failure F2: stored-energy launch",
        lab_name="lab01",
        config_path="configs/lab01_msd/f2_launch_high_energy.yaml",
        plots="essential",
        description="A far target with high stiffness stores 0.5*k*delta^2 before motion starts.",
        try_this="Predict the peak speed with delta*sqrt(k/m), then check the velocity plot.",
        watch="A velocity spike near 10 m/s and an oscillation that barely decays.",
    ),
    MenuAction(
        group="Lab01 Mass-Spring-Damper",
        label="Failure F2: safe counterpart",
        lab_name="lab01",
        config_path="configs/lab01_msd/f2_launch_precheck.yaml",
        plots="essential",
        description="The same 0.5 m displacement made safe by a smaller energy budget.",
        try_this="Compare stored energy (3.75 J vs 50 J) and peak speed against the launch case.",
        watch="A small velocity peak and a clean settle with almost no residual energy.",
    ),
    MenuAction(
        group="Lab02 PID Control",
        label="Auto demo",
...
def launch_learning_path_latest_folder(
    step: LearningPathStep,
    outputs_root: Path | None = None,
) -> subprocess.Popen[Any] | None:
    latest = learning_path_latest_output(step, outputs_root)
...
def launch_latest_folder(latest_output: dict[str, Path | None]) -> subprocess.Popen[Any] | None:
    path = latest_output.get("path")
    if path is None:
        return None
    return open_path(path)
...
def main() -> int:
    ...
    path_folder_buttons: list[tuple[LearningPathStep, Any]] = []
    ...
    def refresh_learning_path_progress() -> None:
        ...
        for step, button in path_folder_buttons:
            update_learning_path_button_text(step, button, "folder")
            button.state(["!disabled"] if learning_path_latest_output(step) is not None else ["disabled"])
    ...
    for step_index, step in enumerate(LEARNING_PATH):
        ...
        artifact_second_row = ttk.Frame(cell)
        artifact_second_row.pack(anchor="w", pady=(4, 0))
        step_folder_button = ttk.Button(
            artifact_second_row,
            text=artifact_labels["folder"],
            width=8,
            command=lambda selected=step: launch_learning_path_latest_folder(selected),
        )
        if learning_path_latest_output(step) is None:
            step_folder_button.state(["disabled"])
        step_folder_button.pack(side="left")
        path_folder_buttons.append((step, step_folder_button))
    ...
    for action_index, action in enumerate(BATCH_ACTIONS):
        ...
        folder_button = ttk.Button(
            ...
            text="Folder",
            width=10,
            command=lambda selected=action: launch_action_latest_folder(selected),
        )
        if batch_latest is None:
            folder_button.state(["disabled"])
        folder_button.pack(anchor="w", pady=(4, 0))
    ...
    latest_folder_button = ttk.Button(
        bottom,
        text=latest_labels["folder"],
        command=lambda: launch_latest_folder(latest_output),
    )
    ...
    def render_actions(*_args: Any) -> None:
        ...
        for group, actions in grouped.items():
            ...
            for action_index, action in enumerate(actions):
                ...
                if latest is None:
                    folder_button.state(["disabled"])
                folder_button.grid(row=action_index, column=6, sticky="w", padx=(0, 12), pady=4)
                ...
                replay_button.grid(row=action_index, column=7, sticky="w", padx=(0, 12), pady=4)
                ...
                if isinstance(followup, BatchMenuAction) and batch_readiness(followup).status != "ok":
                    next_button.state(["disabled"])
                next_button.grid(row=action_index, column=8, sticky="w", padx=(0, 12), pady=4)
                ...
                compare_button.grid(row=action_index, column=9, sticky="w", padx=(0, 12), pady=4)
                ttk.Label(frame, text=lesson_text(action), wraplength=500, justify="left").grid(
                    row=action_index, column=10, sticky="w", pady=4
                )
        canvas.configure(scrollregion=canvas.bbox("all"))

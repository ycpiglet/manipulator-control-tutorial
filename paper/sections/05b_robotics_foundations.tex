...
\input{figures/fig_two_link_ik_branches}
...
\input{figures/fig_two_link_jacobian_columns}
...
\end{align}
둘째 식의 두 항이 어디서 오는지도 곱의 미분 규칙으로 볼 수 있다.
속도 식은 $\frac{\dd\bm{q}}{\dd s}$와 $\dot{s}$의 곱이므로, 시간으로 한 번 더 미분하면 앞 인자를 미분한 항과 뒤 인자를 미분한 항이 하나씩 나온다.
앞 인자 $\frac{\dd\bm{q}}{\dd s}$는 $s$의 함수라서 연쇄법칙으로 $\frac{\dd^2\bm{q}}{\dd s^2}\dot{s}$가 되고 여기에 남아 있던 $\dot{s}$가 곱해져 $\dot{s}^2$ 항이 되며, 뒤 인자 $\dot{s}$의 미분에서 $\ddot{s}$ 항이 나온다.
...
\end{equation}
가 된다.
여기서 \(\dd^2\bm{x}/\dd s^2=\bm{0}\)인 이유도 한 줄이면 된다.
\(\bm{x}(s)=\bm{x}_0+s\Delta\bm{x}\)를 \(s\)로 미분하면 상수 벡터 \(\Delta\bm{x}\)가 나오고, 상수를 한 번 더 미분하면 0이기 때문이다.
...
\input{figures/fig_path_trajectory_profiles}

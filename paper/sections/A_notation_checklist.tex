...
$V(s)$는 전압이지만, $V_x(s)$, $V_e(s)$, $V_d(s)$처럼 아래첨자가 붙으면 속도의 라플라스 변환으로 읽고, $V_d(s)$는 움직이는 목표점의 속도이다.
...
\begin{table}[H]
  ...
  \begin{tabularx}{\linewidth}{@{}>{\raggedright\arraybackslash}p{0.16\linewidth}>{\raggedright\arraybackslash}p{0.26\linewidth}>{\raggedright\arraybackslash}p{0.15\linewidth}>{\raggedright\arraybackslash}X@{}}
    ...
    $\dot{x}(t)$, $V_x(s)$, $V_e(s)$, $V_d(s)$ & 속도 & $\mathrm{m/s}$ & 아래첨자가 붙은 $V_x$, $V_e$, $V_d$는 전압이 아니라 속도이고, $V_d$는 움직이는 목표점의 속도이다 \\
